...
def extract_links(html_content):
    ...
    for link in links:
        url, title = link
        # Full URLs in Wikipedia are relative (e.g., /wiki/...), so we add the domain
...
def process_news_item(item_html):
    ...
    # Extract the text inside the <li> tags
    ...
    images = extract_images(item_html)  # Extract images from the news item
...
def main():
    ...
    if html_content:
        ...
        # Save the processed news to a JSON file
        output = {
        ...
        print("News data with links and images processed and saved to 'processed_news.json'")
    else:
